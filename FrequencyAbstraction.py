    ...
    def abstract_frequency(self, data_table, columns, window_size, sampling_rate):
        ...
        for col in columns:
            ...
            data_table[col].rolling(
                window_size).apply(self.find_fft_transformation)

            # Pad the missing rows with nans
            frequencies = np.pad(np.array(self.temp_list), ((window_size - 1, 0), (0, 0)),
                        'constant', constant_values=np.nan)
            # add new freq columns to frame
            
            frequencies_df = pd.DataFrame(frequencies, columns=collist, index=data_table.index)

            data_table = pd.concat([data_table, frequencies_df], axis=1)
